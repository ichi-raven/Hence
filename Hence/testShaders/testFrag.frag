...
void main()
{
    vec2 mo = shadertoy.iMouse.xy / shadertoy.iResolution.xy;
	float time = 32.0 + shadertoy.iTime*1.5;
    vec2 fragCoord = vec2(inUV.x * shadertoy.iResolution.x, (1.0 - inUV.y) * shadertoy.iResolution.y);

    // camera	
    vec3 ta = vec3( 0.25, -0.75, -0.75 );
    vec3 ro = ta + vec3( 4.5*cos(0.2*time + 7.0*mo.x), 2.2, 4.5*sin(0.2*time + 7.0*mo.x) );
    // camera-to-world transformation
    mat3 ca = setCamera( ro, ta, 0.0 );

    vec3 tot = vec3(0.0);
#if AA>1
    for( int m=ZERO; m<1; m++ )
    for( int n=ZERO; n<1; n++ )
    {
        // pixel coordinates
        vec2 o = vec2(float(m),float(n)) / float(AA) - 0.5;
        vec2 p = (2.0*(fragCoord+o)-shadertoy.iResolution.xy)/shadertoy.iResolution.y;
#else    
        vec2 p = (2.0*fragCoord-shadertoy.iResolution.xy)/shadertoy.iResolution.y;
#endif
        ...
        vec3 rd = ca * normalize( vec3(p,fl) );
         ...
         // ray differentials
        vec2 px = (2.0*(fragCoord+vec2(1.0,0.0))-shadertoy.iResolution.xy)/shadertoy.iResolution.y;
        vec2 py = (2.0*(fragCoord+vec2(0.0,1.0))-shadertoy.iResolution.xy)/shadertoy.iResolution.y;
