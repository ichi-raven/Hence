...
#define detail .00004
#define t 2.0+shadertoy.iTime*.2
...
vec4 formula(vec4 p) {
		p.xz = abs(p.xz+1.5+sin(shadertoy.iTime*0.1*p.z)*0.1)-abs(p.xz-1.5+sin(shadertoy.iTime*0.01*p.z)*0.1)-p.xz;
...
vec4 formula2(vec4 p) {
		p.xz = abs(p.xz+1.5+cos(shadertoy.iTime*0.1+p.z)*0.1)-abs(p.xz-1.5+sin(shadertoy.iTime*0.1+p.z)*0.1)-p.xz;
...
void main()
{
   ...
   vec2 q = fragCoord.xy / shadertoy.iResolution.xy;
   if (q.y < .12 || q.y >= .88) {
		fragColor=vec4(0.,0.,0.,1.);
		return;
	}

    lightdir.y = lightdir.y +shadertoy.iTime*0.01;

	pth1 = path(t+.3)+origin;
	vec2 uv = fragCoord.xy / shadertoy.iResolution.xy*2.-1.;
	uv.y*=shadertoy.iResolution.y/shadertoy.iResolution.x;
